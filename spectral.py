...
class _MSCohEst(_CohEstBase):
    ...
    def compute_con(self, con_idx, n_epochs, psd_xx, psd_yy):  # lgtm
        ...
        self.con_scores[con_idx] = np.power(np.abs(np.abs(csd_mean) / np.sqrt(psd_xx * psd_yy)),2)
...
_CON_METHOD_MAP = {'mscoh' : _MSCohEst, 'coh': _CohEst, 'cohy': _CohyEst, 'imcoh': _ImCohEst,
...
def _prepare_connectivity(epoch_block, times_in, tmin, tmax,
    ...
    freqs = freqs_all[freq_mask] #break point here HERE
    #freqs = [5.859375, 7.812500, 9.765625, 11.718750, 13.671875, 15.625000, 17.578125, 19.531250, 21.484375, 23.437500, 25.390625, 27.343750, 29.296875, 31.250000, 33.203125]
    n_freqs = len(freqs)
    ...
    #freqs_bands = [np.array([5.859375, 7.812500, 9.765625, 11.718750, 13.671875, 15.625000, 17.578125, 19.531250, 21.484375, 23.437500, 25.390625, 27.343750, 29.296875, 31.250000, 33.203125])]

    # make sure we don't have empty bands
    for i, n_f_band in enumerate([len(f) for f in freqs_bands]): #break point here
        if n_f_band == 0:
